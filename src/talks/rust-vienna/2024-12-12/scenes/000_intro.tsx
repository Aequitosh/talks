...
export default makeScene2D(function* (view) {
  ...
  const titleTo = "Strong Types,\nStronger Guarantees";

  const subtitle = createRef<Txt>();
  const subtitleTo = "Compile-Time Checking Techniques";
  ...
  yield* beginSlide("goals_compile_time_checks");

  subtitle().padding([vh(10), 0, 0, 0]);
  subtitle2().padding([vh(10), 0, 0, 0]);

  let nextSubtitle = "getting Rust to check things\nfor you during compilation";
  let nextSubtitle2 = "";

  yield* subtitle().text(nextSubtitle, 1.2, easeInOutQuad);

  yield* beginSlide("not_just_borrow_checker");

  nextSubtitle2 = "-> beyond the borrow checker";

  subtitle2().opacity(1);
  yield* subtitle2().text(nextSubtitle2, 0.8, easeInOutQuad);

  yield* beginSlide("learning_from_the_industry");

  nextSubtitle2 = "-> learning from the industry";

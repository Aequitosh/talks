...
export default makeScene2D(function*(view) {
  ...
  yield* all(
    chain(
      ...
      all(
        title().opacity(1, 1),
        title().text(NOBREAK_SPACE.repeat(titleTo.length), 0).to(titleTo, 1.25, easeInOutQuad),
      ),
    ),
    ...
    chain(
      ...
      all(
        ...
        subtitle().text(NOBREAK_SPACE.repeat(subtitleTo.length), 0).to(subtitleTo, 2.5, easeInOutQuad),
      ),
    ),
  );
  ...
  yield* all(
    ...
    chain(
      ...
      all(
        name().opacity(1, 1),
        name().text(nameTo, 2.5, easeInOutQuad),
      )
    ),
